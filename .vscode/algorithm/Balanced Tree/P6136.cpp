...
ll getpre(int rt, ll key) //查找小于key的最大数
{
    if (!rt) return -INF;
...
void solve()
{
    ...
    for (int i = 1; i <= m; i ++)
    {
        opt = read(), x = read(), x = x ^ last;
        if (opt == 1) //插入一个数
            insert(root, x);
        else if (opt == 2) //删除一个数
            remove(root, x);
        else if (opt == 3) //查询一个数的排名
            last = getrank(root, x) - 1, ans ^= last; //减一是因为插入了一个负无穷大的哨兵
        else if (opt == 4) //查询第k大的数
            last = kth(root, x + 1), ans ^= last; //同样是因为负无穷大的哨兵
        else if (opt == 5) //查询小于x的最大数
            last = getpre(root, x), ans ^= last;
        else if (opt == 6) //查询大于x的最小数
            last = getnxt(root, x), ans ^= last;
    }
    printf("%lld\n", ans);
}
