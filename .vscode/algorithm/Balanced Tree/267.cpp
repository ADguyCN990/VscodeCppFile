...
int new_node(ll key) //新建一个节点，相当于new一个指针
{
    int now = ++idx;
...
void insert(int &now, ll key) //插入一个值为key的值，因为涉及到根的修改所以加个引用
{
...
void remove(int &now, ll key) //删除一个值为key的值，因为涉及到根的修改所以加个引用
{
...
int getrank(int rt, ll key) //查询值为x的排名
{
    if (!rt) return 0;
...
ll getpre(int rt, ll key) //查找小于等于key的最大数
{
    if (!rt) return -INF;
    if (key == tree[rt].key) return tree[rt].key;
    else if (key < tree[rt].key) return getpre(tree[rt].l, key);
    else return max(tree[rt].key, getpre(tree[rt].r, key));
}

ll getnxt(int rt, ll key) //查找大于key的最小数
{
    if (!rt) return INF;
    if (key == tree[rt].key) return tree[rt].key;
...
void solve()
{
    ...
    build();
    ll tmp;
    tmp = read(), insert(root, tmp);
    ll ans = tmp;
    for (int i = 1; i < n; i ++)
    {
        tmp = read();       
        ll contribute;
        contribute = min(tmp - getpre(root, tmp), getnxt(root, tmp) - tmp);
        //printf("!!! %lld\n", contribute);
        ans += contribute;
        insert(root, tmp);
    }
